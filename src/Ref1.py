...
for VIDEO_ID in common.VIDEO_ID_LIST:
    ...
    for TMP_HTRACE_ID in range(common.Num_head_trace[VIDEO_ID]):
        ...
        # continue
        # 
        file_result_all = "Training_Result/"+"Hidden_" + str(num_of_hidden) +"/VID_" + str(VIDEO_ID) + "_HTRACE_ID_" + str(HTRACE_ID) + "_Ref1.txt"
        ...
        # EST_WIN_LIST = np.array([30])
            ...
            for DELAY in DELAY_LIST:
                ...
                for idx_Test in range(0,TestT):
                    ...
                    # viewport estimation
                    # phi
                    est_VP_phi, err_VP_phi = ref.last_phi(video_test, Label_test, Num_sess_test, Num_seg_max, DELAY, EST_WIN)
                    # theta
                    est_VP_theta, err_VP_theta = ref.last_theta(video_test_theta, Label_test_theta, Num_sess_test, Num_seg_max, DELAY, EST_WIN)
                    # 
                    ...
                    # combine
                    # result_full
                    file_result_per_frame = "Training_Result/"+"Hidden_" + str(num_of_hidden) +"/VID_" + str(VIDEO_ID)+ "_HTRACE_ID_" + str(HTRACE_ID) + "_Ref1_DELAY_" + str(DELAY) + "_ESTWIN_" + str(EST_WIN) + "_per_frame.txt"
                    ...
                    # result_short
                    file_result_per_frame_short = "Training_Result/"+"Hidden_" + str(num_of_hidden) +"/VID_" + str(VIDEO_ID)+ "_HTRACE_ID_" + str(HTRACE_ID) + "_Ref1_DELAY_" + str(DELAY) + "_ESTWIN_" + str(EST_WIN) + "_per_frame_short.txt"
                    ...
                    result_file_all.write("0\t%d\t%d\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n" %(EST_WIN, DELAY, rmse_phi, avg_err_phi, CI_phi, std_phi, rmse_theta, avg_err_theta, CI_theta, std_theta, rmse, avg_err, CI, std, np.mean(black_tile_ratio), np.mean(visi_tile_BR_ratio), np.mean(redun_tile_num)/common.NO_TILE, np.mean(redun_tile_ratio)))
